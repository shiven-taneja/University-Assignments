...
def train_mle_estimator(train_images, train_labels):
    ...
    theta_mle = np.dot(train_images.T, train_labels)/np.sum(train_labels,
                                                            axis=0)
    pi_mle = np.sum(train_labels, axis=0)/train_images.shape[0]
...
def train_map_estimator(train_images, train_labels):
    ...
    # YOU NEED TO WRITE THIS PART
    theta_map = (2 + np.dot(train_images.T, train_labels))/(4 +
                                                            np.sum(train_labels,
                                                                   axis=0))

    pi_map = np.sum(train_labels, axis=0)/train_images.shape[0]
...
def log_likelihood(images, theta, pi):
    ...
    # YOU NEED TO WRITE THIS PART
    num = np.log(pi) + np.dot(images, np.log(theta))

    denom = sp.logsumexp(num, axis=1)

    log_like = (num.T - denom).T
...
def predict(log_like):
    ...
    # YOU NEED TO WRITE THIS PART
    predictions = (log_like == log_like.max(axis=1, keepdims=True))
    return predictions
